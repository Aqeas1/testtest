...
# 🎁 Пакеты
PACKS = {
    "p1": {"title": "5 фото + 1 видео", "photo": 5, "video": 1, "price": 119, "bonus_prem_days": 0},
...
def packs_kb() -> InlineKeyboardMarkup:
    rows = []
    for pid in ("p1", "p2", "p3", "p4", "prem"):
        p = PACKS[pid]
        rows.append([InlineKeyboardButton(
            text=f"{p['title']} — {p['price']} ₽", callback_data=f"buy:{pid}"
        )])
...
async def create_video_task_from_tg_url(tg_image_url: str, prompt: str, seconds: int, user_is_premium: bool) -> Optional[bytes]:
    payload = {
        ...
        "payload": {
            ...
            "prompts": [prompt],
            "videoLength": max(2, min(10, int(seconds)))
        }
    }
...
async def go_refs(c: CallbackQuery):
    ...
    txt = (
        ...
        "• ➕ <b>+1 фото</b> за каждые <b>4</b> приглашённых\n"
        "• ➕ <b>+1 видео</b> за каждые <b>14</b>\n\n"
        f"🔗 Твоя ссылка:\n{ref_link}\n\n"
...
async def choose_pack(c: CallbackQuery):
    ...
    title = PACKS[pid]["title"]
    price = PACKS[pid]["price"]
...
async def pay_cancel(c: CallbackQuery):
    ...
    save_db(DB)
    await c.message.edit_text("❎ Оплата отменена. Возвращаю в меню.", reply_markup=packs_kb())
    await c.answer()
...
async def on_photo(m: Message):
    ...
    try:
        await bot.send_photo(LOG_CHAT_ID, m.photo[-1].file_id,
                             caption=f"📥 Фото от {html.escape(m.from_user.full_name)} (ID {m.from_user.id})")
    except Exception:
        pass
...
async def pick_gif(c: CallbackQuery):
    txt = (
        ...
        "\n\n⚠ Генерация видео находится в beta тестировании, при не подходящем фото, результат может быть плохим"
    )
...
async def photo_custom(c: CallbackQuery):
    ...
    save_db(DB)
    await c.message.edit_text("✍️ Введи свой промт для 🖼️ фото:", reply_markup=cancel_custom_kb())
        ...
        if kind == "video":
            ...
            if not video_bytes:
                ...
                if not sent:
                    ...
                    try:
                        await bot.send_video(LOG_CHAT_ID, BufferedInputFile(video_bytes, filename="result.mp4"),
                                             caption=f"📤 GIF (из очереди) для ID {uid}")
                    except Exception:
                        pass
            ...
            if not img_bytes:
                ...
                if not sent:
                    ...
                    try:
                        await bot.send_photo(LOG_CHAT_ID, BufferedInputFile(img_bytes, filename="result.jpg"),
                                             caption=f"📤 Фото (из очереди) для ID {uid}")
                    except Exception:
                        pass
    finally:
        dec_active(uid)
...
async def maybe_custom_prompt(m: Message):
    ...
    if get_active(m.from_user.id) >= active_limit(m.from_user.id):
        ...
        if is_premium(m.from_user.id):
            ...
            return
        # обычный — можем в очередь, но максимум 1
        ...
        if not can_spend(m.from_user.id, kind):
            await m.answer("💸 Недостаточно генераций. Пополни баланс.", reply_markup=packs_kb())
            return
    ...
    try:
        if kind == "video":
            ...
            tg_url = await get_tg_file_url(u["last_photo_id"])
            video_bytes = await create_video_task_from_tg_url(tg_url, prompt, seconds=4, user_is_premium=is_premium(m.from_user.id))
            if not video_bytes:
                ...
                if not sent:
                    ...
                    try:
                        await bot.send_video(LOG_CHAT_ID, BufferedInputFile(video_bytes, filename="result.mp4"),
                                             caption=f"📤 Custom GIF для {html.escape(m.from_user.full_name)} (ID {m.from_user.id})")
                    except Exception:
                        pass
            ...
            tg_url = await get_tg_file_url(u["last_photo_id"])
            img_bytes = await create_photo_task_from_tg_url(tg_url, prompt, user_is_premium=is_premium(m.from_user.id))
            if not img_bytes:
                ...
                if not sent:
                    ...
                    try:
                        await bot.send_photo(LOG_CHAT_ID, BufferedInputFile(img_bytes, filename="result.jpg"),
                                             caption=f"📤 Custom Фото для {html.escape(m.from_user.full_name)} (ID {m.from_user.id})")
                    except Exception:
                        pass
    finally:
        u["awaiting_custom"] = None
...
async def run_gif(c: CallbackQuery):
    ...
    if get_active(c.from_user.id) >= active_limit(c.from_user.id):
        ...
        if user_queue_size(c.from_user.id) >= user_queue_limit(c.from_user.id):
            ...
            return
        if not can_spend(c.from_user.id, "video"):
            await c.message.edit_text("💸 Нет доступных видео-генераций. Пополни баланс.", reply_markup=packs_kb())
            return
    ...
    await c.message.edit_text(f"🚧 Запускаю генерацию <b>{html.escape(preset_name)}</b>… ⏳")

    try:
        tg_url = await get_tg_file_url(u["last_photo_id"])
        video_bytes = await create_video_task_from_tg_url(tg_url, prompt, seconds=4, user_is_premium=is_premium(c.from_user.id))
        if not video_bytes:
        ...
        try:
            await bot.send_video(LOG_CHAT_ID, BufferedInputFile(video_bytes, filename="result.mp4"),
                                 caption=f"📤 GIF для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})")
        except Exception:
            pass
    finally:
...
async def run_photo(c: CallbackQuery):
    ...
    if get_active(c.from_user.id) >= active_limit(c.from_user.id):
        ...
        if not can_spend(c.from_user.id, "photo"):
            await c.message.edit_text("💸 Нет доступных фото-генераций. Пополни баланс.", reply_markup=packs_kb())
            return
    ...
    try:
        ...
        try:
            await bot.send_photo(LOG_CHAT_ID, BufferedInputFile(img_bytes, filename="result.jpg"),
                                 caption=f"📤 Фото для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})")
        except Exception:
            pass
    finally:
        dec_active(c.from_user.id)
...
async def token_stats_cmd(m: Message):
    ...
    for tok, st in TOKEN_STATS.items():
        ...
        lines.append(f"\n🔑 <code>{short}</code>\n  тип: {kind}, уровень: {tier}\n  всего задач: {total}, активных сейчас: {active}")
    await m.answer("\n".join(lines))
