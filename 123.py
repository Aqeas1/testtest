...
TELEGRAM_TOKEN = "8327480504:AAETADDJ7OGmHdELLuiRVaG4qtMBtocBBZA"
ADMIN_ID = 7510524298
...
EVENT_CHANNEL_ID = -1003422217520  # канал для постов ивента
...
GIF_8_1 = "The girl takes off all her clothes, exposes her breasts and remains naked, she does it all quickly"
GIF_8_1_2 = "The girl takes off her pants and panties, when she takes them off, you can see her naked body, you can see her naked vagina"

GIF_8_2_1 = "A man's penis appears with an Erection and puts the penis in the girl's mouth, and the girl calmly takes the penis deeper into her mouth and looks at the man from below to above, the man inserts the penis deeper into the girl's mouth several times"
GIF_8_2_2 = "The girl continues to take the man's penis in her mouth, lick it deeper and take it deeper in her mouth"
...
PH_27 = "Change the photo so that the girl is naked, and next to the girl's chest there is a man licking the girl's nipples with his tongue"
PH_28 = "Change the photo so that the girl is completely naked, her legs are spread and sperm is flowing out of her vagina"
PH_29 = "Change the photo so the girl is wearing a tight top and bike shorts that hug her ass, the girl is turned sideways with her ass towards us"
PH_30 = "Change the photo so the girl is completely naked, her legs are spread, and there is a squirt/water spraying out of her vagina. Don't change the facial ratio, leave it as is, don't touch the face."
PH_31 = "Change the photo so that the girl's face does not change, her body remains the same, the photo expands slightly and next to the girl add three men who cum on the girl's clothes from the penis, one ends up on the pants, the second ends up on the chest, the third on the face"
...
PHOTO_PRESETS = [
    ...
    ("👅🍒Раздеть+облизать соски девушки", PH_27),
    ("👚🍑Обтягивающая одежда+повернуться жопой", PH_29),
    ("💦Сквирт", PH_30),
    ("👨💦Кончают на девушку", PH_31),
]
...
def load_db() -> Dict[str, Any]:
    if not os.path.exists(DB_PATH):
        return {
            ...
            "event": None,                    # данные текущего ивента (если есть)
        }
...
def ensure_user(uid: int, ref: Optional[int] = None):
    ...
    if str(uid) not in DB["users"]:
        DB["users"][str(uid)] = {
            ...
            "broadcast_text": None,
            "broadcast_payload": None,  # тут будет храниться шаблон рассылки (текст/фото/видео)
            # 🔽 новое
            "purchases_count": 0,       # сколько раз человек что-то покупал
            "effects_state": None,      # состояние мастера /effects
            "event_state": None,        # состояние мастера /event
        }
        ...
        u.setdefault("awaiting_broadcast", False)
        u.setdefault("broadcast_text", None)
        u.setdefault("broadcast_payload", None)
        u.setdefault("event_state", None)
    save_db(DB)
...
def gif_presets_kb(uid: int) -> InlineKeyboardMarkup:
    ...
    # кастомные эффекты админа (видео)
    video_eff = DB["user_effects"].get("video", [])
    if video_eff:
        # заголовок (текстовая кнопка, ни к чему не ведёт)
        rows.append([
            InlineKeyboardButton(text="⭐ Эффекты админа (видео)", callback_data="noop")
        ])
        for e in video_eff:
            rows.append([
                InlineKeyboardButton(
                    text=f"⭐ {e['name']}",
                    callback_data=f"gif_u:{e['id']}"
                )
            ])

        # разделитель
        rows.append([
            InlineKeyboardButton(text="— стандартные эффекты —", callback_data="noop")
        ])
...
def photo_presets_kb(uid: int) -> InlineKeyboardMarkup:
    ...
    # кастомные эффекты админа (фото)
    photo_eff = DB["user_effects"].get("photo", [])
    if photo_eff:
        rows.append([
            InlineKeyboardButton(text="⭐ Эффекты админа (фото)", callback_data="noop")
        ])
        for e in photo_eff:
            rows.append([
                InlineKeyboardButton(
                    text=f"⭐ {e['name']}",
                    callback_data=f"photo_u:{e['id']}"
                )
            ])
...
async def create_video_task_from_tg_url(
    tg_image_url: str,
    prompt_or_prompts,
    seconds: int,
    ...
    # prompt_or_prompts может быть:
    # - строкой
    # - кортежем/списком из двух промтов (для 8 сек)
    if isinstance(prompt_or_prompts, (list, tuple)):
        p1 = str(prompt_or_prompts[0])
        p2 = str(prompt_or_prompts[1]) if len(prompt_or_prompts) > 1 else str(prompt_or_prompts[0])
    else:
        p1 = str(prompt_or_prompts)
        p2 = p1
...
async def on_photo(m: Message):
    ...
    if is_admin(m.from_user.id) and u.get("awaiting_broadcast"):
        ...
        await m.answer(
            f"📄 <b>Превью рассылки</b> (фото + подпись):\n\n{caption or '— без подписи —'}\n\nОтправить?",
            reply_markup=kb
        )
        return
    ...
    if u.get("awaiting_receipt"):
        ...
        try:
            msg = await bot.send_photo(ADMIN_ID, fid, caption=caption, reply_markup=None)
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"admin:approve:{msg.message_id}")],
                [InlineKeyboardButton(text="❌ Отклонить", callback_data=f"admin:reject:{msg.message_id}")],
                [InlineKeyboardButton(text="⛔ Заблокировать", callback_data=f"admin:block:{msg.message_id}")]
            ])
            await bot.edit_message_reply_markup(chat_id=ADMIN_ID, message_id=msg.message_id, reply_markup=kb)

            DB["receipts"][str(msg.message_id)] = {
                "user": m.from_user.id,
                "pack_id": pid,
                "status": "pending"
            }
...
async def on_video(m: Message):
    ...
    if is_admin(m.from_user.id) and u.get("awaiting_broadcast"):
        ...
        await m.answer(
            f"📄 <b>Превью рассылки</b> (видео + подпись):\n\n{caption or '— без подписи —'}\n\nОтправить?",
            reply_markup=kb
        )
        return

    # для обычных пользователей видео нам не нужно
    if not is_admin(m.from_user.id):
        await m.answer("Сейчас бот работает только с фото 📸. Пришли изображение.")
...
async def gif_custom(c: CallbackQuery):
    ...
    save_db(DB)
    await c.message.edit_text("✍️ Введи свой промт для 🎬 видео:\n\n"
                              "<i>Чтобы видео сделалось корректно, описывайте действия максимально детально\n"
                              "Сложные действия на видео сделаны не будут\n"
                              "Промт желательно писать на английском языке.</i>\n\n"
                              "Пример промта для раздевания: <i>The girl takes off all her clothes, exposes her breasts and remains naked, she does it all quickly</i>",
        ...
        if kind == "video":
            if mode == "preset":
                ...
                if isinstance(raw, list):
                    video_prompts = [str(p or "") for p in raw] or [""]
                else:
                    video_prompts = [str(raw or "")]
            ...
            if not video_bytes:
                ...
                await bot.send_message(chat_id,
                                       "⚠️ Задача из очереди завершилась без результата. Генерация возвращена.")
            else:
                sent = await try_send_video(chat_id, video_bytes, caption="🎬 Готово (очередь)! ✨")
                if not sent:
                    refund(uid, "video")
                    await bot.send_message(chat_id,
                                           "✅ Видео сгенерировано (очередь), но не удалось отправить. Генерация возвращена.")
                else:
                    try:
                        await bot.send_video(
                            ...
                            caption=(
                                ...
                                f"📝 Промт: {html.escape(str((prompt or ''))[:200])}"
                            )
                        )
                    except Exception:
                        pass
...
async def maybe_custom_prompt(m: Message):
    ...
    # ---------- Режим рассылки /rek (текст) ----------
    if is_admin(m.from_user.id) and u.get("awaiting_broadcast"):
        if not text:
            await m.answer("Текст пустой. Пришли текст поста или фото/видео с подписью.")
            return
    ...
    if is_admin(m.from_user.id):
        est = u.get("event_state")
        if est:
            step = est.get("step")

            # ввод начала
            if step == "ask_start":
                dt_utc = parse_event_datetime_msk(text)
                if not dt_utc:
                    await m.answer(
                        "⏱ Не смог разобрать дату.\n"
                        "Формат: <code>ДД.ММ.ГГГГ ЧЧ:ММ</code>, пример: <code>27.11.2025 16:10</code>"
                    )
                    return
                est["start_utc"] = dt_utc.isoformat()
                est["step"] = "ask_end"
                u["event_state"] = est
                save_db(DB)
                await m.answer(
                    "✅ Время начала сохранено.\n\n"
                    "Теперь введи <b>время окончания</b> в том же формате:\n"
            ...
            # ввод конца
            if step == "ask_end":
                dt_end_utc = parse_event_datetime_msk(text)
                if not dt_end_utc:
                    await m.answer(
                        "⏱ Не смог разобрать дату окончания.\n"
                        "Формат: <code>ДД.ММ.ГГГГ ЧЧ:ММ</code>"
                    )
                    return

                dt_start_utc = datetime.fromisoformat(est["start_utc"])
                if dt_end_utc <= dt_start_utc:
                    await m.answer("⚠ Время окончания должно быть позже времени начала. Введи конец ещё раз.")
                    return

                # создаём ивент
                baseline = {}
                for uid_str in DB["users"].keys():
                    baseline[uid_str] = DB["refs"].get(uid_str, {}).get("total", 0)
                ...
                # для красоты показываем время в МСК
                start_msk = dt_start_utc + timedelta(hours=3)
                end_msk = dt_end_utc + timedelta(hours=3)

                await m.answer(
                    "🎉 Ивент запущен!\n\n"
                    f"Старт: <b>{start_msk.strftime('%d.%m.%Y %H:%M')}</b> (МСК)\n"
                    f"Финиш: <b>{end_msk.strftime('%d.%m.%Y %H:%M')}</b> (МСК)\n\n"
                    "Во время ивента бот каждые 10 минут будет отправлять топ-5 по приглашениям "
                    f"в канал <code>{EVENT_CHANNEL_ID}</code>, а в конце начислит призы:\n\n"
                    "1 место: 15 фото + 3 видео\n"
                    "2 место: 10 фото + 2 видео\n"
                    "3 место: 5 фото + 1 видео\n"
                    "4 место: 4 фото + 1 видео\n"
                    "5 место: 3 фото"
                )
                return
...
def gif_length_user_kb(eff_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="⏱ 4 секунды (1 видео)",
                callback_data=f"giflen_u:4:{eff_id}"
            )
        ],
        [
            InlineKeyboardButton(
                text="⏱ 8 секунд (2 видео)",
                callback_data=f"giflen_u:8:{eff_id}"
            )
        ],
        [
            InlineKeyboardButton(
...
def gif_user_length_kb(eff_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(
                text="⏱ 4 секунды (1 видео)",
                callback_data=f"gifulen:4:{eff_id}"
            )
        ],
        [
            InlineKeyboardButton(
                text="⏱ 8 секунд (2 видео)",
                callback_data=f"gifulen:8:{eff_id}"
            )
        ],
        [
            InlineKeyboardButton(
...
# ===================== USER PRESET GIF (ЭФФЕКТЫ АДМИНА) =====================

@dp.callback_query(F.data.startswith("gif_u:"))
async def gif_user_choose_length(c: CallbackQuery):
    try:
        eff_id = int(c.data.split(":", 1)[1])
    except ValueError:
        await c.answer("Некорректный эффект", show_alert=True)
        return

    eff = None
    for e in DB["user_effects"].get("video", []):
        if e["id"] == eff_id:
            eff = e
            break

    if not eff:
        await c.answer("Эффект не найден", show_alert=True)
        return
    ...
    await c.message.edit_text(
        ...
        "• 8 секунд — спишется <b>2</b> видео-генерации\n\n"
        "⚠ Генерация видео находится в beta-тестировании, при неподходящем фото результат может быть плохим.",
        reply_markup=gif_user_length_kb(eff_id)
    )
...
async def run_gif(c: CallbackQuery):
    ...
    preset_name, base_prompt = GIF_PRESETS[idx]

    # подбираем промты для 8 секунд
    if seconds == 8:
        if idx == 0:
            # первый эффект (раздеть)
            prompt_param = (GIF_8_1, GIF_8_1_2)
        elif idx == 1:
            # второй эффект (минет)
            prompt_param = (GIF_8_2_1, GIF_8_2_2)
        else:
            # на всякий случай — дублируем базовый
            prompt_param = (base_prompt, base_prompt)
    else:
        # 4 секунды — один промт
        prompt_param = base_prompt
...
@dp.callback_query(F.data.startswith("gifulen:"))
async def run_gif_user(c: CallbackQuery):
    """
    Запуск user-видео-эффекта после выбора длины.
    callback_data: gifulen:<seconds>:<eff_id>
    """
    parts = c.data.split(":")
    if len(parts) != 3:
        await c.answer("Некорректные данные", show_alert=True)
    ...
    # Лимит активных задач — очередь для этих эффектов не используем, просто ждём
    if get_active(c.from_user.id) >= active_limit(c.from_user.id):
        await c.message.edit_text(tasks_limit_message(c.from_user.id))
        return
    ...
    preset_name = eff["name"]
    p1 = eff.get("prompt1") or ""
    p2 = eff.get("prompt2") or None

    if not p1:
        await c.answer("У эффекта нет промта", show_alert=True)
        return
    ...
    await c.message.edit_text(
        f"🚧 Запускаю ⭐ <b>{html.escape(preset_name)}</b>… ⏳\n"
        f"⏱ Длительность: <b>{seconds} с</b> "
        f"(списано {cost} видео-генерац{'ию' if cost == 1 else 'ии'})"
    )
    ...
    try:
        ...
        video_bytes = await create_video_task_from_tg_url(
            ...
            seconds=seconds,
            user_is_premium=is_premium(c.from_user.id),
            prompt2=p2 if seconds > 4 else None
        )
        if not video_bytes:
            # вернём все списанные генерации
            for _ in range(cost):
        ...
        sent = await try_send_video(c.message.chat.id, video_bytes, caption=f"🎬 Готово! ⭐ {preset_name}")
        if not sent:
            for _ in range(cost):
                refund(c.from_user.id, "video")
            await c.message.edit_text("✅ Сгенерировано, но не удалось отправить видео. Генерация возвращена.")
        ...
        try:
            await bot.send_video(
                ...
                BufferedInputFile(video_bytes, filename="result.mp4"),
                caption=(
                    f"📤 GIF (user-эффект) для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})\n"
                    f"✨ Эффект: {html.escape(preset_name)}\n"
                    f"⏱ Длительность: {seconds} с\n"
                    f"🎟 Списано генераций: {cost}\n"
                    f"🧩 ID эффекта: {eff_id}"
...
@dp.callback_query(F.data.startswith("giflen_u:"))
async def run_gif_user(c: CallbackQuery):
    parts = c.data.split(":")
    if len(parts) != 3:
        await c.answer("Некорректные данные", show_alert=True)
    ...
    eff = None
    for e in DB.get("user_effects", {}).get("video", []):
        if e.get("id") == eff_id:
            eff = e
            break
    ...
    name = eff.get("name", f"Эффект {eff_id}")
    prompt1 = eff.get("prompt1", "")
    prompt2 = eff.get("prompt2") or prompt1
    ...
    if get_active(c.from_user.id) >= active_limit(c.from_user.id):
        ...
        # обычный — очередь, НО 8сек не ставим в очередь
        if seconds > 4:
            await c.message.edit_text(
                "⏳ Лимит активных задач достигнут.\n\n"
                "8-секундные видео нельзя ставить в очередь — "
        ...
        # ставим в очередь как custom, но передаём 2 промта списком
        spend(c.from_user.id, "video")
        enqueue_user_task(
            uid=c.from_user.id,
            kind="video",
    ...
    await c.message.edit_text(
        f"🚧 Запускаю генерацию <b>{html.escape(name)}</b>… ⏳\n"
        f"⏱ Длительность: <b>{seconds} с</b> "
        f"(списано {cost} видео-генерац{'ию' if cost == 1 else 'ии'})"
    )
    ...
    try:
        tg_url = await get_tg_file_url(u["last_photo_id"])
        prompts_list = [prompt1] if seconds == 4 else [prompt1, prompt2]
        video_bytes = await create_video_task_from_tg_url(
            tg_url,
            prompts_list,
            seconds=seconds,
        ...
        try:
            await bot.send_video(
                ...
                BufferedInputFile(video_bytes, filename="result.mp4"),
                caption=(
                    f"📤 GIF (user-effect) для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})\n"
                    f"✨ Эффект: {html.escape(name)}\n"
                    f"⏱ Длительность: {seconds} с\n"
                    f"🎟 Списано генераций: {cost}"
                )
            )
...
@dp.callback_query(F.data.startswith("photo_u:"))
async def run_photo_user(c: CallbackQuery):
    try:
        eff_id = int(c.data.split(":", 1)[1])
    except Exception:
        await c.answer("Некорректный ID эффекта", show_alert=True)
        return
    ...
    eff = None
    for e in DB.get("user_effects", {}).get("photo", []):
        if e.get("id") == eff_id:
            eff = e
            break
    ...
    await c.message.edit_text(f"🚧 Делаю <b>{html.escape(name)}</b>… ⏳")
    ...
    try:
        ...
        try:
            await bot.send_photo(
                ...
                BufferedInputFile(img_bytes, filename="result.jpg"),
                caption=(
                    f"📤 Фото (user-effect) для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})\n"
                    f"✨ Эффект: {html.escape(name)}"
                )
            )
        except Exception:
        ...
        dec_active(c.from_user.id)
        await maybe_run_from_queue(c.from_user.id, c.message.chat.id)
# ===================== USER PRESET PHOTO (ЭФФЕКТЫ АДМИНА) =====================
...
async def run_photo_user(c: CallbackQuery):
    ...
    # ищем эффект в базе
    eff = None
    for e in DB["user_effects"].get("photo", []):
        if e["id"] == eff_id:
            eff = e
            break
    ...
    # Лимит активных задач — для этих эффектов очередь НЕ используем, просто просим подождать
    if get_active(c.from_user.id) >= active_limit(c.from_user.id):
        await c.message.edit_text(tasks_limit_message(c.from_user.id))
        return
    ...
    try:
        ...
        sent = await try_send_photo(c.message.chat.id, img_bytes, caption=f"🖼️ Готово! ⭐ {preset_name}")
        if not sent:
            refund(c.from_user.id, "photo")
            await c.message.edit_text("✅ Сгенерировано, но не удалось отправить фото. Генерация возвращена.")
            return
        ...
        try:
            await bot.send_photo(
                ...
                BufferedInputFile(img_bytes, filename="result.jpg"),
                caption=(
                    f"📤 Фото (user-эффект) для {html.escape(c.from_user.full_name)} (ID {c.from_user.id})\n"
                    f"✨ Эффект: {html.escape(preset_name)}\n"
                    f"🧩 ID эффекта: {eff_id}"
                )
            )
...
async def admin_decision(c: CallbackQuery):
    ...
    if len(parts) >= 2 and parts[1] in {"rek_send", "rek_cancel"}:
        ...
        if parts[1] == "rek_send":
            payload = u.get("broadcast_payload")
            ...
            for uid_str in list(DB["users"].keys()):
                ...
                try:
                    uid = int(uid_str)
                    if payload and payload.get("type") == "photo":
                        await bot.send_photo(
                            uid,
                            payload["file_id"],
                            caption=payload.get("caption") or ""
                        )
                    elif payload and payload.get("type") == "video":
                        await bot.send_video(
                            uid,
                            payload["file_id"],
                            caption=payload.get("caption") or ""
                        )
                    elif payload and payload.get("type") == "text":
                        await bot.send_message(uid, payload.get("text") or text)
                    else:
                        # старый режим, вдруг чего
    ...
    rec = DB["receipts"].get(str(msg_id))
    if not rec:
        await c.answer("Чек не найден", show_alert=True)
        return
    ...
    if action == "approve":
        ...
        # отмечаем чек и чистим pending_orders (если есть)
        rec["status"] = "approved"
        ...
        try:
            await bot.send_message(
                uid,
                "❌ Оплата не подтверждена. Пришли корректный чек или напиши в поддержку.",
                reply_markup=main_menu_kb()
            )
        except Exception:
            pass
...
async def give_photo(m: Message):
    ...
    try:
        ...
        u["photo_credits"] += cnt
        save_db(DB)
        await m.answer(f"✅ Выдал {cnt} фото пользователю {uid}. Новый баланс: {u['photo_credits']} фото.")

        # уведомление пользователю
        try:
            await bot.send_message(
                uid,
                f"🎁 Тебе начислено <b>{cnt}</b> фото-генерац{'ия' if cnt == 1 else 'ии'}.\n"
                f"Твой баланс: 📷 <b>{u['photo_credits']}</b> | 🎞 <b>{u['video_credits']}</b>"
            )
        except Exception:
            pass
...
def parse_event_datetime_msk(s: str) -> Optional[datetime]:
    """
    Ожидает строку вида '27.11.2025 16:10'
    Возвращает время в UTC (мы храним в utc).
    """
    try:
        s = s.strip()
...
def compute_event_top(event: Dict[str, Any], limit: int = 5) -> List[Tuple[int, int]]:
    """
    Возвращает список (user_id, приглашений_за_ивент), отсортированный по убыванию.
    """
    baseline = (event or {}).get("baseline_refs") or {}
    scores: List[Tuple[int, int]] = []

    for uid_str in DB["users"].keys():
        total_refs = DB["refs"].get(uid_str, {}).get("total", 0)
        base = baseline.get(uid_str, 0)
        gained = max(0, total_refs - base)
        if gained > 0:
            scores.append((int(uid_str), gained))

    scores.sort(key=lambda x: x[1], reverse=True)
    return scores[:limit]

@dp.message(Command("event"))
async def event_cmd(m: Message):
    if not is_admin(m.from_user.id):
        return
    u = get_user(m.from_user.id)
    u["event_state"] = {"step": "ask_start"}
    save_db(DB)
    await m.answer(
        "🎯 Настройка ивента по рефералам.\n\n"
        "Введи <b>время начала</b> в формате <code>ДД.ММ.ГГГГ ЧЧ:ММ</code> (по МСК).\n"
        "Например: <code>27.11.2025 16:10</code>"
    )
...
async def give_video(m: Message):
    ...
    try:
        ...
        u["video_credits"] += cnt
        save_db(DB)
        await m.answer(f"✅ Выдал {cnt} видео пользователю {uid}. Новый баланс: {u['video_credits']} видео.")

        # уведомление пользователю
        try:
            await bot.send_message(
                uid,
                f"🎁 Тебе начислено <b>{cnt}</b> видео-генерац{'ия' if cnt == 1 else 'ии'}.\n"
                f"Твой баланс: 📷 <b>{u['photo_credits']}</b> | 🎞 <b>{u['video_credits']}</b>"
            )
        except Exception:
            pass
...
async def event_loop():
    """
    Каждую минуту проверяет ивент:
    - если идёт: каждые 10 минут шлёт промежуточный топ-5
    - если закончился и ещё не обработан: объявляет победителей, начисляет призы
    """
    while True:
        try:
            ev = DB.get("event")
            ...
            start_utc = datetime.fromisoformat(ev["start_utc"])
            end_utc = datetime.fromisoformat(ev["end_utc"])
            ...
            if start_utc <= now_utc <= end_utc:
                last_str = ev.get("last_broadcast_utc")
                need_broadcast = False
                if not last_str:
                    need_broadcast = True
                else:
                    last = datetime.fromisoformat(last_str)
                    if now_utc - last >= timedelta(minutes=10):
                        need_broadcast = True

                if need_broadcast:
                    top = compute_event_top(ev, limit=5)
                    if not top:
                        text = (
                            "🎯 Ивент по приглашениям — промежуточный итог\n\n"
                            "Пока ещё никто никого не пригласил 🤷‍♂️\n\n"
                            "Успей ворваться в топ!"
                        )
                    else:
                        lines = ["🎯 Ивент по приглашениям — топ-5 на данный момент:\n"]
                        for i, (uid, gained) in enumerate(top, start=1):
                            lines.append(
                                f"{i}) <a href=\"tg://user?id={uid}\">Участник {uid}</a> — <b>{gained}</b> приглашений"
                            )
                        text = "\n".join(lines)

                    try:
                        await bot.send_message(EVENT_CHANNEL_ID, text)
                    except Exception:
                        pass

                    ev["last_broadcast_utc"] = now_utc.isoformat()
                    save_db(DB)

                await asyncio.sleep(60)
                continue

            # ивент закончился, но ещё не обработан
            if now_utc > end_utc and not ev.get("finished"):
                top = compute_event_top(ev, limit=5)
                ...
                if not top:
                    ...
                    try:
                        await bot.send_message(
                            EVENT_CHANNEL_ID,
                            "🎯 Ивент по приглашениям завершён!\n\n"
                            "К сожалению, за время ивента не было ни одного приглашения 😔"
                        )
                    except Exception:
                        pass
                    ev["finished"] = True
                ...
                lines = ["🎉 Ивент по приглашениям завершён! Итоговый топ-5:\n"]
                for place, (uid, gained) in enumerate(top, start=1):
                    photo_prize, video_prize = prizes.get(place, (0, 0))
                    u = get_user(uid)
                    u["photo_credits"] += photo_prize
                    u["video_credits"] += video_prize
                    save_db(DB)

                    lines.append(
                        f"{place}) <a href=\"tg://user?id={uid}\">Участник {uid}</a> — "
                        f"<b>{gained}</b> приглашений "
                        f"(приз: 📷 {photo_prize} | 🎞 {video_prize})"
                    )
                    ...
                    try:
                        await bot.send_message(
                            uid,
                            f"🎉 Ты занял <b>{place}</b> место в ивенте по приглашениям!\n"
                            f"Ты привёл <b>{gained}</b> человек.\n"
                            f"Приз: 📷 <b>{photo_prize}</b> | 🎞 <b>{video_prize}</b>."
                        )
                    except Exception:
                        pass

                try:
                    await bot.send_message(EVENT_CHANNEL_ID, "\n".join(lines))
                except Exception:
                    pass
            ...
            log.exception("event_loop error: %s", e)
...
async def give_prem(m: Message):
    if not is_admin(m.from_user.id):
        return
    try:
        _, uid_s, days_s = m.text.strip().split()
        uid = int(uid_s)
        days = int(days_s)
        ...
        # уведомление пользователю
        try:
            until = u.get("premium_until") or ""
            until_str = until[:10] if until else ""
            await bot.send_message(
                uid,
                f"👑 Тебе выдали Premium на <b>{days}</b> дней!\n"
                f"Статус Premium: активен до <b>{until_str}</b>."
            )
        except Exception:
            pass
...
async def cmd_rek(m: Message):
    ...
    await m.answer(
        "📝 Режим рассылки.\n\n"
        "Пришли <b>текст</b> сообщения или <b>фото/видео с подписью</b>, "
        "которые нужно отправить всем пользователям. Потом спрошу, точно ли отправлять."
    )
